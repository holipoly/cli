...
const debug = Debug('holipoly-cli');
...
if (!semver.satisfies(process.versions.node, '>= 16')) {
  console.error(
    `${chalk.red('ERROR')}: Holipoly CLI requires Node.js 16.x or later`,
  );
  process.exit(1);
}
...
if (ExecutionContext.isNot('development') && SentryDSN) {
  const release = `holipoly-cli@${pkg.version}`;
  const dsn = SentryDSN;
...
const parser = yargs(hideBin(process.argv))
  ...
  .command(['backup [command]'], 'Manage Holipoly Cloud backups', backup)
  .command(['task [command]'], 'Manage Holipoly Cloud tasks', task)
  .command(['job [command]'], 'Manage Holipoly Cloud tasks', task)
  .command(['project [command]'], 'Manage Holipoly Cloud projects', project)
  .command(
  ...
  .command(['app [command]'], 'Manage Holipoly Apps', app)
  ...
  .option('instance', {
    alias: ['u', 'url'],
    type: 'string',
    desc: 'Holipoly instance API URL (must start with the protocol, i.e. https:// or http://)',
  })
  .strictCommands()
  ...
  .fail(async (msg, error, _yargs) => {
    ...
    if (error instanceof HTTPError) {
      ...
      console.error(' https://github.com/holipoly/holipoly-cli/issues');
      console.error('\n CLI Docs:');
      console.error(' https://docs.holipoly.io/docs/3.x/cli');
    } else {
