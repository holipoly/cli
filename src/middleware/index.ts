...
const debug = Debug('holipoly-cli:middleware');
...
export const useToken = async ({ token }: Options) => {
  ...
  if (!token) {
    ...
    if (process.env.HOLIPOLY_CLI_TOKEN) {
      debug('token read from env');
      opts = { ...opts, token: `TOKEN ${process.env.HOLIPOLY_CLI_TOKEN}` };
      return opts;
    }
    ...
    console.error(
      chalk(
        ...
        chalk.bold.green('holipoly login'),
      ),
    );
    console.error(
      chalk(
        'If you don\'t have an account - register using',
        chalk.bold.green('holipoly register'),
      ),
    );
...
export const useAppConfig = async (_argv: Options) => {
  try {
    const content = await fs.readFile(
      path.join(process.cwd(), 'holipoly.config.json'),
      'utf-8',
    );
    const config = JSON.parse(content);
    const instance = config.HolipolyInstanceURL;
      ...
      if (isNotFound(error)) {
        spinner.fail(
          chalk.red(
            ...
            environment,
            `- not found in the scope of '${organization}' organization in the Holipoly Cloud`,
          ),
        );
        process.exit(1);
      }
...
export const useEnvironment = async ({
  ...
  if (!token) {
    throw chalk(
      ...
      'to authenticate Holipoly CLI to Holipoly Cloud',
    );
  }
  ...
  if (!organization) {
    throw chalk(
      ...
      chalk.green('holipoly organization switch'),
...
`;
  ...
  if (!argv.email && !argv.password) {
    const { email } = await enquirer.prompt<{ email: string }>({
      type: 'text',
      name: 'email',
      message: 'Your Holipoly Dashboard email?',
    });
...
export const interactiveHolipolyApp = async (argv: Options) => {
  if (!argv.app) {
    const app = await promptHolipolyApp(argv);
  ...
  if (!process.env.CI) {
    try {
      ...
      console.error(
        `You are ${chalk.red(
          'offline',
        )}. Holipoly CLI requires Internet access to operate. Check your connection.`,
      );
      process.exit(1);
    }
  }
...
export const useGithub = async () => {
  ...
  if (!githubToken) {
    ...
    console.log(
      chalk(
        'Run',
        chalk.bold.green('holipoly github login'),
        'command to login',
      ),
    );
...
export const useVercel = async () => {
  ...
  if (!vercelToken) {
    ...
    console.log(
      chalk(
        'Run',
        chalk.bold.green('holipoly vercel login'),
        'command to login',
      ),
    );
...
${pending}
 Use ${chalk.green('holipoly job list')} command to list jobs`,
    );
  }
