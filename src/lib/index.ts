...
const debug = Debug('holipoly-cli:lib:index');

export const defaultCloudApiUrl = 'https://cloud.holipoly.io/platform/api';
export const defaultCloudApiAuthDomain = 'auth.holipoly.io';
...
export const getCloudApiUrl = async () => {
  ...
  return process.env.HOLIPOLY_CLI_ENV_URL || defaultCloudApiUrl;
};
...
export const getCloudApiAuthDomain = async () => {
  ...
  return process.env.HOLIPOLY_CLI_ENV_AUTH_DOMAIN || defaultCloudApiAuthDomain;
};
...
export const getEnvironment = async () => {
  const { holipoly_env: holipolyEnv, token } = await Config.get();
  if (token) {
    return holipolyEnv;
  }

  return process.env.HOLIPOLY_CLI_ENV || 'production';
};
...
export const DefaultHolipolyEndpoint = 'https://vercel.holipoly.cloud/graphql/';
