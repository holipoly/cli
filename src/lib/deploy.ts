...
const debug = Debug('holipoly-cli:lib:deploy');
...
const createProjectInGithub = async (
  ...
  if (!githubProjectCreate) {
    throw new HolipolyAppError('Holipoly App deployment cancelled by the user');
  }
  ...
  if (errors) {
    for (const error of errors) {
      if (error.message === 'Name already exists on this account') {
        ...
        throw new HolipolyAppError(error.message);
      }
    }
  } else {
    const {
