...
const debug = Debug('holipoly-cli:env:create');
...
export const builder: CommandBuilder = (_) =>
  _.positional('name', {
    ...
    .option('holipoly', {
      type: 'string',
      desc: 'specify the Holipoly version',
    })
    .option('domain', {
      type: 'string',
    ...
    .example('holipoly env create my-environment', '')
    .example(
...
export const createEnvironment = async (argv: Arguments<CommandOptions>) => {
  const { project, holipoly, database } = argv;

  debug('getting user from Holipoly API');
  const _user = (await GET(API.User, argv)) as User;

  if (argv.restore && !argv.restoreFrom) {
    throw new HolipolyEnvironmentError(
