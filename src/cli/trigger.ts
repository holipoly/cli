...
import * as HolipolyGraphQL from '../generated/graphql.js';
import { Config } from '../lib/config.js';
import { DefaultHolipolyEndpoint } from '../lib/index.js';
import { capitalize, obfuscateArgv, HolipolyEventError } from '../lib/util.js';
import { useAppConfig, useInstanceConnector } from '../middleware/index.js';
import { Options } from '../types.js';

const debug = Debug('holipoly-cli:trigger');

export const command = 'trigger [event]';
export const desc = 'This triggers a Holipoly event';
...
export const handler = async (argv: Arguments<Options>) => {
  ...
  const {
    ...
    DefaultHolipolyEndpoint,
    HolipolyGraphQL.GetWebhookEventEnum,
  );
  ...
  if (!choices.map((_) => _.name).includes(event.toUpperCase())) {
    throw new HolipolyEventError('Wrong event name');
  }
  ...
  if (!(operationName in HolipolyGraphQL)) {
    throw new HolipolyEventError('Operation not implemented');
  }
  ...
  try {
    const result = await request(
      instance,
      (HolipolyGraphQL as any)[operationName],
      {
        id,
        input: {},
      },
