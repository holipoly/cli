...
export const desc = 'Setup an official Holipoly example locally';
...
export const builder: CommandBuilder = (_) =>
  _.positional('name', {
    ...
    .example(
      'holipoly example auth-sdk',
      'Setup the auth-sdk example from holipoly/examples on GitHub',
    );
...
export const handler = async (argv: Arguments<any>): Promise<void> => {
  ...
  if (!template) {
    const examples = await getRepositoryContent(
      'https://api.github.com/repos/holipoly/examples/contents',
    );
  ...
  const repository = `https://github.com/holipoly/example-${tmpl}.git`;
  ...
  spinner.succeed(
    chalk(
      'Your Holipoly example is ready in the',
      chalk.yellow(target),
      'directory\n',
    ),
  );
