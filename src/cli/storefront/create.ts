...
const debug = Debug('holipoly-cli:storefront:create');
...
export const builder: CommandBuilder = (_) =>
  _.positional('name', {
    ...
    .option('template', {
      type: 'string',
      default: Config.HolipolyStorefrontRepo,
      alias: ['t'],
    })
    .option('branch', {
      type: 'string',
      default: Config.HolipolyStorefrontDefaultBranch,
      alias: 'b',
    });
...
const createProject = async (argv: Arguments<StoreCreate>) => {
  ...
  const demoName = capitalize(argv.name || 'holipoly demo');
...
const prepareEnvironment = async (
  ...
  printlnSuccess(
    chalk(
      chalk.bold('Select a Holipoly version ·'),
      chalk.cyan(
        `Holipoly ${service.version} - ${service.display_name} - ${service.service_type}`,
      ),
    ),
  );
...
export const createStorefront = async (argv: Arguments<StoreCreate>) => {
  ...
  await replace.replaceInFile({
    files: 'package.json',
    from: /("name": "(react|holipoly)-storefront").*/g,
  ...
  if (instance) {
    ...
    await replace.replaceInFile({
      files: '.env',
      from: /NEXT_PUBLIC_HOLIPOLY_API_URL=.*/g,
      to: `NEXT_PUBLIC_HOLIPOLY_API_URL=${instance}`,
    });
  }
  ...
  spinner.succeed(
    chalk(
      'Your Holipoly Storefront Example is ready in the',
      chalk.yellow(target),
      'directory\n',
    ),
  );
  ...
  if (!instance) {
    ...
    println(
      '    set NEXT_PUBLIC_HOLIPOLY_API_URL value to your Holipoly GraphQL endpoint URL\n',
    );
  }

  println(chalk.bold('  To setup payments:\n'));
  println(
    '    install and configure the Holipoly payment App in your dashboard',
  );
  if ((instance || '').includes('holipoly.cloud/graphql/')) {
    println(`    ${instance.replace('graphql/', '')}dashboard/apps/`);
  }

  println(
    chalk('\n', chalk.bold(' To start your Holipoly Storefront Example:\n')),
  );
  println(`    cd ${target}`);
  println('    pnpm dev\n');
};
