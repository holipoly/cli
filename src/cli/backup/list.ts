...
const debug = Debug('holipoly-cli:backup:list');
...
export const builder: CommandBuilder = (_) =>
  _.option('name', {
    ...
    .example('holipoly backup list', '')
    .example('holipoly backup list --organization="organization-slug"', '');
...
export const handler = async (argv: Arguments<Options>) => {
  ...
  cli.table(result, {
    ...
    version: {
      header: 'Ver.',
      minWidth: 2,
      get: ({ holipoly_version: version }) => chalk.yellow(version),
    },
    created: {
      minWidth: 2,
