...
const debug = Debug('holipoly-cli:task:list');
...
export const builder: CommandBuilder = (_) =>
  _.option('env', { type: 'string' })
    ...
    .example('holipoly task list', '')
    .example('holipoly task list my-environment --page=2', '')
    .example('holipoly task list my-environment --page-size=100', '')
    .example('holipoly task list my-environment --is-blocking', '')
    .example('holipoly task list my-environment --status=active', '');
  ...
  if (name === 'job') {
    contentBox(
      chalk(
        ...
        'please use',
        chalk.green('holipoly task list'),
        'command',
      ),
    );
  }
