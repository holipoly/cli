...
import { interactiveHolipolyApp } from '../../middleware/index.js';
import { Options, WebhookError } from '../../types.js';

const debug = Debug('holipoly-cli:webhook:create');
...
export const handler = async (argv: Arguments<Options>) => {
  ...
  } = await request<any>(DefaultHolipolyEndpoint, GetWebhookAsyncEventEnum);
  const asyncEventsListChoices = asyncEventsList.filter(without('ANY_EVENTS'));

  const {
    __type: { enumValues: syncEventsList },
  } = await request<any>(DefaultHolipolyEndpoint, GetWebhookSyncEventEnum);
...
export const middlewares = [interactiveHolipolyApp];
