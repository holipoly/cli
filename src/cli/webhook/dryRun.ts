...
export const builder: CommandBuilder = (_) =>
  _.option('object-id', {
    ...
    .example('holipoly webhook dry-run', '')
    .example(
      'holipoly webhook dry-run --query=\'subscription { event { ... on ProductCreated { product { id name } } } }\'',
      '',
    )
    .example('holipoly webhook dry-run --object-id=\'UHJvZHVjdDo3Mg==\'', '');
...
export const handler = async (argv: Arguments<WebhookDryRunArgs>) => {
  ...
  try {
    ...
    if (error instanceof HTTPError) {
      ...
      if (statusCode === 400) {
        throw new Error(
          'Seems the selected environment doesn\'t support dry run feature. Required Holipoly Version ^3.11',
        );
      }
    }
