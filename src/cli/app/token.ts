...
const debug = Debug('holipoly-cli:app:token');

export const command = 'token';
export const desc = 'Create a Holipoly App token';
...
export const builder: CommandBuilder = (_) =>
  _.option('app-id', {
    ...
    .example('holipoly app token', '')
    .example('holipoly app token --app-id="app-id"', '')
    .example(
...
export const handler = async (argv: Arguments<Options>) => {
  ...
  if (!argv.appId) {
    const { app } = await getHolipolyApp({ instance, json });
    appId = app!;
  } else {
...
export const getHolipolyApp = async ({
  instance,
  appId = undefined,
  json,
}: {
  ...
  const { app } = await Enquirer.prompt<{ app: string }>({
    ...
    name: 'app',
    choices,
    message: 'Select a Holipoly App (start typing) ',
  });
