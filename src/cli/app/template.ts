...
const debug = Debug('holipoly-cli:app:template');

export const command = 'template [name]';
export const desc = 'Create an App with Holipoly App Template';

export const builder: CommandBuilder = (_) =>
  _.positional('name', {
    type: 'string',
    demandOption: true,
    default: 'my-holipoly-app',
  })
    .option('dependencies', {
      type: 'boolean',
      default: true,
    ...
    .option('template', {
      type: 'string',
      default: Configs.HolipolyAppRepo,
      alias: ['t', 'repo', 'repository'],
    })
    .option('branch', {
      type: 'string',
      default: Configs.HolipolyAppDefaultBranch,
      alias: 'b',
    })
    .option('example', {
      type: 'string',
...
export const handler = async (argv: Arguments<StoreCreate>): Promise<void> => {
  ...
  await checkPnpmPresence('This Holipoly App template');
  ...
  await replace.replaceInFile({
    files: 'package.json',
    from: /"name": "holipoly-app-template".*/g,
    to: `"name": "${packageName}",`,
  });
  ...
  spinner.succeed(
    chalk(
      'Your Holipoly app is ready in the',
      chalk.yellow(target),
      'directory\n',
    ),
  );
  ...
  println(
    chalk(
      '\nTip: use',
      chalk.green('holipoly app tunnel'),
      'to expose your local environment to a public URL and install your app in the Holipoly instance',
    ),
  );
};
...
const getRepositoryContent = async (
  repoPath = 'https://api.github.com/repos/holipoly/app-examples/contents/examples',
) => {
