...
export const buildPath = `${process.env.PWD}/dist/holipoly.js`;
export const testOrganization = 'devtools';
export const testEnvironmentName = 'holipoly-test';
...
export const createTestEnvironment = async (
  ...
  if (!envExists) {
    const params = [
      ...
      `--project=${testProjectName}`,
      '--database=sample',
      '--holipoly=holipoly-master-staging',
      `--domain=holipoly-test-domain-${currentDate()}`,
